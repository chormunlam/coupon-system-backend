...
//import com.example.trade.coupon.service.CouponCodeService;
...
//    private CouponCodeService couponCodeService;
    ...
    public ModelAndView userCouponList(@PathVariable("userId") long userId) {
        ...
        List<Coupon> coupons = couponQueryService.queryUsrCoupons(userId);// all user coupon
        ...
        for (Coupon coupon : coupons) {
            CouponVO couponVo = createCouponVo(coupon);//tranfer
    ...
    public CouponVO createCouponVo(Coupon coupon) {
        ...
        //serach cache firt (redis)
        String batchRule = redisWorker.getValueByKey("couponBatchRule:" + coupon.getBatchId());//get value by key
        //if not in reis, then go database
        if (StringUtils.isEmpty(batchRule)) {
            ...
            redisWorker.setValue("couponBatchRule:" + coupon.getBatchId(), batchRule);
        }
        /*
